(() => {
  const ensureStyles = () => {
    ['./visuals.css','./public/fixes.css','./atlas.css','./hero.css','./brand-v2.css','./images-v2.css','./brand-official.css?v=10','./quantic-sillage.css?v=20260915'].forEach(href => {
  ...
  const logoUrl=new URL('assets/quantic-sillage-logo.svg?v=1',baseUrl).href;
  const markUrl=new URL('assets/quantic-sillage-mark.svg?v=1',baseUrl).href;
  ...
  document.querySelectorAll('.brand').forEach(brand=>{
    ...
    img.alt='Quantic Sillage — Ideas Travel Further';
    img.src=logoUrl;
  ...
  const replaceBrandText = (root=document.body) => {
    ...
    for(const n of nodes){
      if(!n.nodeValue||!/(Quantic Minds|QUANTIC MINDS|Quantic Browser|QUANTIC BROWSER|QuanticOS)/.test(n.nodeValue)) continue;
      n.nodeValue=n.nodeValue
        .replaceAll('QUANTIC MINDS','QUANTIC SILLAGE')
        .replaceAll('Quantic Minds','Quantic Sillage')
        .replaceAll('QUANTIC BROWSER','QUANTIC GLIDE')
        .replaceAll('Quantic Browser','Quantic Glide')
        .replaceAll('QuanticOS','Quantic OS');
    }
  };
  replaceBrandText();
  document.title=document.title.replace(/Quantic Minds/gi,'Quantic Sillage').replace(/Quantic Browser/gi,'Quantic Glide').replace(/QuanticOS/g,'Quantic OS');
  const description=document.querySelector('meta[name="description"]');if(description)description.content=description.content.replace(/Quantic Minds/gi,'Quantic Sillage').replace(/Quantic Browser/gi,'Quantic Glide').replace(/QuanticOS/g,'Quantic OS');
  ...
  document.querySelectorAll('.nav a,.mobile-nav a,.footer-links a').forEach(a=>{const href=(a.getAttribute('href')||'').split('#')[0];if(href==='solutions.html')a.textContent='Produits';if(href==='research.html')a.textContent='Vision'});
  document.querySelectorAll('.future-section .eyebrow').forEach(el=>{if(/NOS PROJETS/i.test(el.textContent))el.textContent='L\'ÉCOSYSTÈME QUANTIC SILLAGE'});
  document.querySelectorAll('.future-section-head>a').forEach(a=>{if(/projets/i.test(a.textContent))a.textContent='Explorer l\'écosystème →'});
