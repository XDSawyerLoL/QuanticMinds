(() => {
  ...
  document.querySelectorAll('.footer-links').forEach(n => {
    if (!n.querySelector('a[href="news.html"]')) {
      const a=document.createElement('a'); a.href='news.html'; a.textContent='Quantic News'; n.appendChild(a);
    }
  });
  ...
  if(subjectParam&&subjectField){[...subjectField.options].forEach(o=>{if(o.value.toLowerCase()===subjectParam.toLowerCase()||o.textContent.toLowerCase()===subjectParam.toLowerCase())subjectField.value=o.value})}
  const form=document.querySelector('[data-contact-form]');
  if(form){form.addEventListener('submit',e=>{e.preventDefault();const fd=new FormData(form);const subject=encodeURIComponent(fd.get('subject')||'Contact Quantic Minds');const body=encodeURIComponent(`Nom : ${fd.get('name')||''}\nEntreprise : ${fd.get('company')||''}\nEmail : ${fd.get('email')||''}\n\n${fd.get('message')||''}`);location.href=`mailto:contact@quanticminds.fr?subject=${subject}&body=${body}`})}

  if(['lab.html','research.html','press.html','careers.html'].includes(file)){
    const hero=document.querySelector('.page-hero');
    if(hero&&!document.querySelector('.visual-showcase')){
      const sec=document.createElement('section');sec.className='visual-showcase';sec.innerHTML=`<div class="visual-showcase-grid"><div class="visual-shot lab-photo"><div class="visual-label">Quantic Minds<b>Le laboratoire</b></div></div><div class="visual-shot team-photo"><div class="visual-label">People<b>Les esprits du Lab</b></div></div><div class="visual-shot product-photo"><div class="visual-label">Products<b>Construire autrement</b></div></div></div>`;hero.insertAdjacentElement('afterend',sec)
    }
  }
  ...
  const buildHomeNews=async()=>{
    ...
    const anchor=document.querySelector('.products-band');if(!anchor||document.querySelector('.news-teaser'))return;
    const sec=document.createElement('section');sec.className='news-teaser';sec.innerHTML=`<div class="wrap"><div class="news-teaser-head"><div class="news-teaser-logo" aria-label="Quantic News"></div><p>Le flux technologique de Quantic Minds : IA, robotique, logiciels, cybersécurité et nouveaux usages.</p><a class="pill-btn" href="news.html">Ouvrir Quantic News →</a></div><div class="news-teaser-grid"><div class="news-empty">Chargement du flux…</div></div></div>`;anchor.insertAdjacentElement('afterend',sec);
    try{const r=await fetch('https://xdsawyerlol.github.io/LEFILLIBRE/feed.json',{cache:'no-store'});if(!r.ok)throw new Error('feed');const data=await r.json();const items=(data.items||[]).filter(i=>i.image&&techMatch(i)).slice(0,3);const grid=sec.querySelector('.news-teaser-grid');grid.innerHTML=items.length?items.map(i=>`<a class="news-mini" href="${esc(i.url)}" target="_blank" rel="noopener"><div class="news-mini-media" style="background-image:url('${esc(i.image)}')"></div><div class="news-mini-copy"><div class="news-mini-meta">${esc(i.source)} · ${timeAgo(i.publishedAt)}</div><h3>${esc(i.title)}</h3><p>${esc((i.summary||'').slice(0,145))}${(i.summary||'').length>145?'…':''}</p><span class="news-mini-more">Lire la source →</span></div></a>`).join(''):'<div class="news-empty">Aucune actualité technologique illustrée disponible pour le moment.</div>'}catch(e){sec.querySelector('.news-teaser-grid').innerHTML='<div class="news-empty">Le flux Quantic News est momentanément indisponible.</div>'}
  };
  buildHomeNews();
})();