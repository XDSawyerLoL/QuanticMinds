(() => {
  ...
  const render=()=>{const list=items.filter(i=>match(i,current));grid.innerHTML=list.length?list.map(i=>`<article class="news-card" data-story="${esc(storyKey(i))}"><div class="news-card-media" style="${i.image?`background-image:linear-gradient(180deg,rgba(2,7,13,.04),rgba(2,7,13,.18)),url('${esc(i.image)}')`:''}"></div><div class="news-card-body"><div class="news-card-meta"><span>${esc(i.source||'Source')}</span><span>•</span><span>${ago(i.publishedAt)}</span>${i.category?`<span>•</span><span>${esc(i.category)}</span>`:''}</div><h2>${esc(i.title)}</h2><p>${esc((i.summary||'').slice(0,260))}${(i.summary||'').length>260?'…':''}</p><div class="news-card-actions"><a class="news-share-linkedin" href="${shareHref(i)}" target="_blank" rel="noopener noreferrer" aria-label="Partager cet article Quantic News sur LinkedIn">in&nbsp; Partager</a><a class="news-source-link" href="${esc(i.url)}" target="_blank" rel="noopener">Lire la source →</a></div></div></article>`).join(''):'<div class="news-empty">Aucune actualité technologique ne correspond à ce filtre pour le moment.</div>';focusDeepLink()};
