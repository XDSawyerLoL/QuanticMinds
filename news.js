(() => {
  ...
  loadFeed().then(data=>{items=(data.items||[]).filter(eligible).sort((a,b)=>new Date(b.publishedAt)-new Date(a.publishedAt)).slice(0,60);updated.textContent=data.updatedAt?`${items.length} actus tech · mis à jour ${ago(data.updatedAt)}`:`${items.length} actus tech`;render()}).catch(()=>{updated.textContent='Flux indisponible';grid.innerHTML='<div class="news-empty">Impossible de charger le flux Quantic News pour le moment. Réessaie dans quelques minutes.</div>'});
})();
