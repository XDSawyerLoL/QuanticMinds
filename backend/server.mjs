...
function cors(req){const origin=req.headers.origin;return origin===FRONTEND_ORIGIN?{'access-control-allow-origin':origin,'vary':'Origin','access-control-allow-headers':'authorization, content-type, x-cron-secret','access-control-allow-methods':'GET,PUT,POST,OPTIONS'}:{}}
...
async function handler(req,res){const c=cors(req);if(req.method==='OPTIONS'){res.writeHead(204,c);return res.end()}const url=new URL(req.url,PUBLIC_BASE_URL||'http://localhost');try{
  if(url.pathname==='/health'&&req.method==='GET')return json(res,200,{ok:true,configured:missingConfig().length===0,missing:missingConfig()},c);
